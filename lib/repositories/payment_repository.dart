...
class PaymentRepository {
  // =====================================================
  // EARLIEST PENDING INSTALLMENT FOR A LOAN
  // =====================================================

  Future<LoanPayment?> getEarliestPendingInstallment(int loanId) async {
    final db = await DatabaseHelper.database;
    ...
    return LoanPayment.fromMap(result.first);
  }

  // =====================================================
  // MARK A SCHEDULED INSTALLMENT AS PAID + UPDATE LOAN BALANCE
  // =====================================================
  //
  // Updates the existing PENDING row in place (so its card flips to PAID
  // with the payment date shown) instead of inserting a new row.
  ...
  Future<void> payInstallment({
    ...
    await db.transaction((txn) async {
      ...
      double currentBalance =
          (loanResult.first["remaining_balance"] as num).toDouble();
